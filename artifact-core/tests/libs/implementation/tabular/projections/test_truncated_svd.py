...
@pytest.mark.parametrize(
    ...
    [
        ("dataset_small", TruncatedSVDHyperparams(use_categorical=True)),
        ("dataset_small", TruncatedSVDHyperparams(use_categorical=False)),
        ("dataset_large", TruncatedSVDHyperparams(use_categorical=True)),
        ("dataset_large", TruncatedSVDHyperparams(use_categorical=False)),
        ("dataset_mixed", TruncatedSVDHyperparams(use_categorical=True)),
        ("dataset_mixed", TruncatedSVDHyperparams(use_categorical=False)),
    ],
...
def test_project(
    ...
    patcher_pca = mocker.patch.object(
        target=TruncatedSVD, attribute="fit_transform", return_value=mock_projection
    )
