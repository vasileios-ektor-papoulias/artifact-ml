...
import pandas as pd
import pytest
from artifact_core._libs.resource_specs.table_comparison.protocol import (
    TabularDataDType,
)
...
def test_build(
    ...
    if expected_exception is not None:
        ...
        assert spec.features == expected_ls_features, (
            f"Expected ls_features: {expected_ls_features} got {spec.features}"
        )
        assert spec.n_features == expected_n_features, (
            f"Expected n_features: {expected_n_features} got {spec.n_features}"
        )
        assert spec.cts_features == expected_ls_cts_features, (
            f"Expected ls_cts_features: {expected_ls_cts_features} got {spec.cts_features}"
        )
        ...
        assert spec.cts_dtypes == expected_dict_cts_dtypes, (
            f"Expected dict_cts_dtypes: {expected_dict_cts_dtypes} got {spec.cts_dtypes}"
        )
        assert spec.cat_features == expected_ls_cat_features, (
            f"Expected ls_cat_features: {expected_ls_cat_features} got {spec.cat_features}"
        )
        ...
        assert spec.cat_dtypes == expected_dict_cat_dtypes, (
            f"Expected dict_cat_dtypes: {expected_dict_cat_dtypes} got {spec.cat_dtypes}"
        )
        ...
        assert spec.seq_n_cat == expected_ls_n_cat, (
            f"Expected ls_n_cat: {expected_ls_n_cat} got {spec.seq_n_cat}"
        )
...
def test_fit(
    ...
    spec = TabularDataSpec.build(cts_features=ls_cts_features, cat_features=ls_cat_features)
    if expected_exception is not None:
        expected_exception_type = type(expected_exception)
        expected_exception_message = expected_exception.args[0]
        with pytest.raises(
...
def test_serialization_deserialization(
    ...
    assert deserialized_spec.features == spec.features
    assert deserialized_spec.n_features == spec.n_features
    assert deserialized_spec.cts_features == spec.cts_features
    assert deserialized_spec.n_cts_features == spec.n_cts_features
    assert set(deserialized_spec.cts_dtypes.keys()) == set(spec.cts_dtypes.keys())
    for key in spec.cts_dtypes:
        assert deserialized_spec.cts_dtypes[key] == spec.cts_dtypes[key]
    assert deserialized_spec.cat_features == spec.cat_features
    assert deserialized_spec.n_cat_features == spec.n_cat_features
    assert set(deserialized_spec.cat_dtypes.keys()) == set(spec.cat_dtypes.keys())
    for key in spec.cat_dtypes:
        assert deserialized_spec.cat_dtypes[key] == spec.cat_dtypes[key]
    ...
    assert deserialized_spec.seq_n_cat == spec.seq_n_cat
...
def test_export_load(
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        assert loaded_spec.features == spec.features
        assert loaded_spec.n_features == spec.n_features
        assert loaded_spec.cts_features == spec.cts_features
        assert loaded_spec.n_cts_features == spec.n_cts_features
        assert set(loaded_spec.cts_dtypes.keys()) == set(spec.cts_dtypes.keys())
        for key in spec.cts_dtypes:
            assert loaded_spec.cts_dtypes[key] == spec.cts_dtypes[key]

        assert loaded_spec.cat_features == spec.cat_features
        assert loaded_spec.n_cat_features == spec.n_cat_features
        assert set(loaded_spec.cat_dtypes.keys()) == set(spec.cat_dtypes.keys())
        for key in spec.cat_dtypes:
            assert loaded_spec.cat_dtypes[key] == spec.cat_dtypes[key]

        assert loaded_spec.cat_unique_map == spec.cat_unique_map
        assert loaded_spec.cat_unique_count_map == spec.cat_unique_count_map
        assert loaded_spec.seq_n_cat == spec.seq_n_cat
