...
def test_descriptive_statistics_compute(
    ...
    if expect_raise_missing:
        with pytest.raises(ValueError, match="Missing columns"):
            TableStatsCalculator.compute(df=df, cts_features=ls_cts_features, stat=stat)
    ...
    if expect_raise_missing:
        with pytest.raises(ValueError, match="Missing columns"):
            TableStatsCalculator.compute_juxtaposition(
                df_real=df_real, df_synthetic=df_synthetic, cts_features=ls_cts_features, stat=stat
            )
    elif expect_raise_unsupported:
        with pytest.raises(ValueError, match="Unsupported"):
            TableStatsCalculator.compute_juxtaposition(
