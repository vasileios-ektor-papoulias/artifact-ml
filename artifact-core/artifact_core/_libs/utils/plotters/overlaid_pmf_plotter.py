...
class OverlaidPMFPlotter:
    ...
    def plot_overlaid_pmf(
        ...
        if ls_unique_categories is None:
            ls_unique_categories = cls._get_ls_unique_categories(
        ...
        bar_centerpoints = np.arange(len(ls_unique_categories))
        bar_width = config.cat_pmf_bar_width
        sr_freq_a = cls._get_sr_freq(sr_data=sr_data_a, ls_unique_categories=ls_unique_categories)
        sr_freq_b = cls._get_sr_freq(sr_data=sr_data_b, ls_unique_categories=ls_unique_categories)
        fig, ax = plt.subplots()
        plt.close(fig)
        ax.bar(
            x=bar_centerpoints - bar_width / 2,
            height=sr_freq_a.values,
            width=bar_width,
        ...
        ax.set_xticklabels(labels=ls_unique_categories, rotation=config.rotation)
    ...
    @staticmethod
    def _get_sr_freq(sr_data: pd.Series, ls_unique_categories: List[str]) -> pd.Series:
