...
class PlotCombiner:
    @classmethod
    def combine(cls, dict_plots: Dict[str, Figure], config: PlotCombinationConfig) -> Figure:
        nfigs = len(dict_plots)
        if nfigs == 0:
            ...
            combined_fig = cls._get_combined_figure(dict_plots=dict_plots, config=config)
    ...
    def _draw_all_content(
        ...
        for ax, (label, fig) in zip(axs, dict_plots.items()):
            cls._draw_component_figure_content(ax=ax, label=label, fig=fig, config=config)

    @classmethod
    def _turn_off_extra_axes(cls, axs: List[Axes], dict_plots: Dict[str, Figure]):
        for extra_ax in axs[len(dict_plots) :]:
            extra_ax.axis("off")
