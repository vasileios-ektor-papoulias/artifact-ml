...
from artifact_experiment._impl.clear_ml.adapter import ClearMLRunAdapter
from artifact_experiment._impl.clear_ml.loggers.array_collections import (
    ClearMLArrayCollectionLogger,
)
from artifact_experiment._impl.clear_ml.loggers.arrays import ClearMLArrayLogger
from artifact_experiment._impl.clear_ml.loggers.files import ClearMLFileLogger
from artifact_experiment._impl.clear_ml.loggers.plot_collections import (
    ClearMLPlotCollectionLogger,
)
from artifact_experiment._impl.clear_ml.loggers.plots import ClearMLPlotLogger
from artifact_experiment._impl.clear_ml.loggers.score_collections import (
    ClearMLScoreCollectionLogger,
)
from artifact_experiment._impl.clear_ml.loggers.scores import ClearMLScoreLogger
...
class ClearMLLoggingWorker(BackendLoggingWorker[ClearMLRunAdapter]):
    ...
    ) -> ArtifactLogger[ClearMLRunAdapter, Score]:
        return ClearMLScoreLogger(run=run)

    @staticmethod
    def _get_array_logger(
        run: ClearMLRunAdapter,
    ) -> ArtifactLogger[ClearMLRunAdapter, Array]:
        return ClearMLArrayLogger(run=run)

    @staticmethod
    def _get_plot_logger(
        run: ClearMLRunAdapter,
    ) -> ArtifactLogger[ClearMLRunAdapter, Plot]:
    ...
    def _get_file_logger(
        run: ClearMLRunAdapter,
    ) -> BackendLogger[ClearMLRunAdapter, File]:
