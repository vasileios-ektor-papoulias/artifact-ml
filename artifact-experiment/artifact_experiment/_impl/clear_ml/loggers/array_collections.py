...
from artifact_experiment._impl.clear_ml.loggers.artifacts import ClearMLArtifactLogger
from artifact_experiment._utils.filesystem.incremental_paths import IncrementalPathGenerator


class ClearMLArrayCollectionLogger(ClearMLArtifactLogger[ArrayCollection]):
    _fmt = ".npz"
    ...
    def _append(self, item_path: str, item: ArrayCollection):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            np.savez_compressed(file=local_filepath, allow_pickle=True, **item)
            self._run.upload(
