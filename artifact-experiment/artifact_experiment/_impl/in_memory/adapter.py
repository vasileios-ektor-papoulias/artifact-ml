...
from artifact_experiment._impl.in_memory.native_run import InMemoryRun
...
class InMemoryRunAdapter(RunAdapter[InMemoryRun]):
    ...
    def search_score_store(self, artifact_path: str) -> Sequence[str]:
        return self._search_store(artifact_path=artifact_path, store=self.scores)

    def search_array_store(self, artifact_path: str) -> Sequence[str]:
        return self._search_store(artifact_path=artifact_path, store=self.arrays)

    def search_plot_store(self, artifact_path: str) -> Sequence[str]:
        return self._search_store(artifact_path=artifact_path, store=self.plots)

    def search_score_collection_store(self, artifact_path: str) -> Sequence[str]:
        return self._search_store(artifact_path=artifact_path, store=self.score_collections)

    def search_array_collection_store(self, artifact_path: str) -> Sequence[str]:
        return self._search_store(artifact_path=artifact_path, store=self.array_collections)

    def search_plot_collection_store(self, artifact_path: str) -> Sequence[str]:
        return self._search_store(artifact_path=artifact_path, store=self.plot_collections)
    ...
    @staticmethod
    def _search_store(artifact_path: str, store: Mapping[str, Any]) -> Sequence[str]:
